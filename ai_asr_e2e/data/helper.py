...
def preprocess_wav(root, dirc, filename):
    ...
    with open(root + "/" + dirc + "/proc_transcript/phaseII/" + filename + ".txt", "r",
        ...
        for line in transcript_file:
            ...
            pipe = subprocess.check_output(
                "sox " + source_audio + " " + target_flac_audio + " trim " + str(start_time) + " " + str(dif_time),
                shell=True)
            try:
                ...
                with open(root + "/parts/" + dirc + "/proc_transcript/" + filename + "_" + str(part_num) + ".txt", "w+",
                          encoding="utf-8") as text_file:
                    text_file.write(text + "\n")
            except:
                print(
                    "Error reading audio file: unknown length, the audio is not with proper length, skip, target_flac_audio {}",
                    target_flac_audio)
...
def traverse(root, path, dev_conversation_phase2, test_conversation_phase2, dev_interview_phase2, test_interview_phase2,
             search_fix=".txt"):
    f_train_list = []
    f_dev_list = []
    f_test_list = []
